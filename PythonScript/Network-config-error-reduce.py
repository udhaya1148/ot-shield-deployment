...
def validate_route(route, gateway):
    ...
    Further optimized by removing redundant checks and using faster subprocess calls.
    """
    try:
        # Get the route table only once
        result = subprocess.check_output(['ip', 'route'], text=True)
        
        # Check if the gateway exists in the routing table
        if gateway and gateway not in result:
            # If the gateway is not found in the routing table, consider it unreachable
            return False, f"Gateway {gateway} is unreachable."

        # Check if the route already exists
        if any(route in line for line in result.splitlines()):
            return False, f"Route {route} already exists."
        
        return True, ""
    except subprocess.CalledProcessError as e:
        return False, f"Error running 'ip route': {e}"
    except Exception as e:
        return False, str(e)
