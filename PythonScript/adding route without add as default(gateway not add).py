...
def update_network():
    ...
    try:
        ...
        if dhcp_enabled:
            ...
            # Ensure a default route is only set if explicitly requested
            if gateway and any(route.get("to") == "default" for routes in routes):
            ...
            # Apply the changes
            updated_routes = [route for route in static_routes if route['to'] != 'default'] + default_routes
  # Merge both correctly
            if updated_routes:
